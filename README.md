...
Interactive CLI for finding recoverable hidden Twitch VOD playlists and downloading them with `ffmpeg`.
...
Settings are stored in the platform-specific user config directory provided by `platformdirs`.
The `settings.toml` file is created with defaults on first run.
